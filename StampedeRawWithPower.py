...
class Pedestrian:
  def __init__(self, location, state):
    ...
    self.power = np.random.rand() # their power score between zero and one

# parameters
board_size = 30 # size of board 
num_people = 200 # number of people
...
Ts = 5 # occurrence time of the stampede
...
while stampede:
    ...
    if time == Ts:
        chaos = True
        """c_list = np.random.choice(range(len(person_list)), size=(2), replace = False)
        for x in c_list:
    ...
    if dupes.size > 0:
        ...
        for dupe in dupes:
            ...
            # NEW FEATURE: possibility to fall when you walk into eachother
            move_p = np.array([person.power for person in people_moving])
            move_p = move_p/sum(move_p)
            picked = np.random.choice(people_moving, p = move_p)
            picked.location = picked.new_location
            people_moving = np.delete(people_moving, people_moving == picked)

            for people in people_moving:
                if people.power < picked.power:
                    power_diff = picked.power - people.power
                    people.state = np.random.choice(['C',people.state], p = [power_diff, 1-power_diff])
                    if people.state == 'C':
                        people.time_down = 0
                        people.direction = [2,2]
                        board[people.location[0], people.location[1]] += 1
